...
// subject directly. These patterns intentionally target activity semantics, not
// incidental words inside a story/definition.
const FORBIDDEN_BY_DOMAIN = {
    reading: /چند تا شکل در تصویر|کفهٔ? ترازو|رنگ «|حیوانات و میوه‌ها را|هر حس را به موقعیت|کارگاه نقاشی|سطل بازیافت/,
    math: /کدام حرف|روی حرف|کدام کلمه|هر کلمه را|واژهٔ درست|هر حس را|در خانواده کیست|حیوانات و میوه‌ها|کارگاه نقاشی|سطل بازیافت/,
    science: /کدام حرف|روی حرف|کدام کلمه|واژهٔ درست|به اضافه|منهای|کارگاه نقاشی|هر حس را به موقعیت/,
    'socio-emotional': /کدام حرف|روی حرف|عدد بعدی|چند تا شکل در تصویر|کفهٔ? ترازو|سایه دقیق|شکل کامل‌کننده|کارگاه نقاشی/,
    art: /کدام حرف|روی حرف|عدد بعدی|کفهٔ? ترازو|چند تا شکل در تصویر|هر حس را به موقعیت|سطل بازیافت/
};
...
for (const domain of curriculum.domains) for (const level of domain.levels) for (const lesson of level.lessons) {
    ...
    for (const age of [4, 5, 6, 7, 8]) for (let pass = 0; pass < passes; pass++) {
        ...
        for (const [roundIndex, round] of rounds.entries()) {
            ...
            const semanticText = `${round.prompt || ''} ${round.speech || ''}`;
            const spoken = String(round.speech || round.prompt || '').trim();
            const mappedSpeechClip = spoken && window.NARRATION_MAP ? window.NARRATION_MAP[spoken] : null;
            const letterClip = String(round.audioClip || '').startsWith('letter-') ? round.audioClip : null;
            if (!mappedSpeechClip && !letterClip) missingNarrationTexts.add(spoken);
            const forbidden = FORBIDDEN_BY_DOMAIN[domain.id];
            if (forbidden && forbidden.test(semanticText)) where(`cross-domain content in ${domain.id}: «${semanticText.slice(0, 100)}»`);
            ...
            if (round.type === 'quiz') {
                ...
                if (round.letter) {
                    ...
                    if (expected && /کدام حرف/.test(prompt) && round.audioClip !== expected) {
                        where(`letter-sound round ${round.letter} uses ${round.audioClip}, expected ${expected}`);
                    }
                }
