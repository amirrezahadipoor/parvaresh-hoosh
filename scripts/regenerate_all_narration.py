...
def runtime_extra_texts():
    ...
    animal_names = ['خرگوش', 'سگ', 'گربه', 'میمون', 'فیل', 'خرس',
                    'لاک‌پشت', 'روباه', 'قورباغه', 'گاو', 'گوسفند', 'اردک']
    turns = [
    ...
    texts.update(f'به {name} چه غذایی بدهیم؟' for name in animal_names)
    count_words = ['صفر', 'یک', 'دو', 'سه', 'چهار', 'پنج', 'شش']
    texts.update(f'{count_words[count]} هدیه داخل واگن قطار بگذار!' for count in range(2, 7))
    ...
    texts.update({
        ...
        'هر چیزی که دوست داری با رنگ‌های شاد نقاشی کن!',
        'هر چیزی که دوست داری با رنگ‌های شاد نقاشی کن، گل من!',
        'صدای او او صدای کدام حیوان است؟',
        'صدای ما ما صدای کدام حیوان است؟',
        'صدای غُرّشِ بلند صدای کدام حیوان است؟',
        'صدای قُد قُد قُدا صدای کدام حیوان است؟',
        'صدای غُرّشِ بَم صدای کدام حیوان است؟',
        'صدای وِز وِز صدای کدام حیوان است؟',
        'بزرگ‌تر مهربان با قصه‌های شیرین و قدیمی'
    })
    return texts


def obsolete_text(text):
    old_food = (' چی غذا بدیم؟' in text)
    old_train = bool(__import__('re').match(r'^[2-6] تا هدیه داخل واگن قطار بذار!$', text))
    old_paint = text in {'هر چی دوست داری با رنگ‌های شاد نقاشی کن!', 'هر چی دوست داری با رنگ‌های شاد نقاشی کن گل من!'}
    return old_food or old_train or old_paint or text == 'بزرگتر مهربان با قصه‌های شیرین و قدیمی'
...
def build_jobs():
    ...
    for text, old_clip in current_text_map().items():
        if not str(old_clip).startswith('letter-') and not obsolete_text(text):
            texts.add(text)
    ...
    for attempt in range(4):
        ...
        try:
            async with semaphore:
                # Edge's native prosody control preserves the neural vocoder output.
                # Post-processing pitch shifts made consonants such as «ث/ف» less clear.
...
async def main():
    ...
    can_reuse_pipeline = (old_ledger.get('voice') == VOICE and old_ledger.get('rate') == RATE
                          and old_ledger.get('edgePitch') == EDGE_PITCH
                          and old_ledger.get('postPitch') == POST_PITCH)
