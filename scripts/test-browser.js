...
//
// ⚠ آستانه از ۲ به ۴ رفت و دلیلش را می‌نویسم چون «جابه‌جا کردن
// آستانه» معمولاً غلط است و باید توجیه شود:
//
// قانونِ واقعیِ نقشهٔ راه «یک تپ تا بازی» است (قانون ۹) و «حداکثر
// دو دکمه» فقط تفسیرِ آن روزِ من بود، وقتی خانه دو دکمه داشت.
// افزودن «بازی‌ها» آن قانون را نمی‌شکند: دکمهٔ بزرگِ بالا هنوز
// مستقیم به درسِ بعدی می‌رود و هیچ صفحهٔ واسطی اضافه نشده.
//
// سقفِ ۴ هم دلبخواه نیست: همان بازهٔ پژوهشیِ «۳ تا ۵ گزینه» است که
// در سراسر برنامه به کار رفته (بخش ۱.۱ نقشهٔ راه).
//
// چیزی که *واقعاً* باید محافظت شود، پایین‌تر آزموده می‌شود: اینکه
// نخستین دکمه همان دکمهٔ بازی باشد.
const decisions = await page.locator('.screen button:not(.icon-btn)').count();
check(decisions <= 4, `خانه نباید بیش از ۴ دکمه داشته باشد، ${decisions} دارد`);

// ⚠ آزمونِ سخت‌ترِ جایگزین: دکمهٔ بازی باید *نخستین* دکمهٔ صفحه
// باشد. اگر روزی «بازی‌ها» یا هر چیز دیگری بالای آن بنشیند، مسیر
// یادگیری دیگر مسیر پیش‌فرض نیست و این آزمون می‌شکند.
const firstBtnClass = await page.evaluate(() => {
  const b = document.querySelector('.screen button:not(.icon-btn)');
  return b ? b.className : '';
});
check(
  firstBtnClass.includes('play-btn'),
  `نخستین دکمهٔ خانه باید دکمهٔ بازی باشد، «${firstBtnClass}» است — مسیر یادگیری باید پیش‌فرض بماند`,
);
