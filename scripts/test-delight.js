...
// ── بازخوردِ صوتیِ لمس ──────────────────────────────────────────────
//
// صدای لمس با یک شنوندهٔ سراسری روی ریشه کار می‌کند و گزینه‌های پاسخ
// را با فهرستی از نامِ کلاس‌ها مستثنا می‌کند.
//
// ⚠ آن فهرست یک بمبِ ساعتی است: نامِ کلاس در فایلِ دیگری تعریف شده و
// اگر کسی `order-tray` را عوض کند، هیچ چیزی نمی‌شکند — فقط صدای لمس
// و صدای «آفرین» روی هم می‌افتند و کسی نمی‌فهمد چرا صدا گِل‌آلود
// شده. دقیقاً همان دستهٔ باگی که این پروژه بارها خورده است:
// *خاموش*. پس اینجا بررسی می‌کنیم هر کلاسِ مستثنا واقعاً در DOM
// وجود داشته باشد.
{
  const src = await (await fetch(`${BASE}/src/ui/screens.js`)).text();
  const m = src.match(/btn\.closest\('([^']+)'\)/);
  if (!m) {
    errors.push('شنوندهٔ سراسریِ صدای لمس پیدا نشد');
  } else {
    const classes = m[1].split(',').map((c) => c.trim().replace(/^\./, ''));
    // هر کلاس باید جایی در کدِ رابط ساخته شود، وگرنه استثنا بی‌اثر است.
    for (const c of classes) {
      const used = new RegExp(`['\`]${c}[\\s'\`$]`).test(src);
      check(used, `کلاسِ «${c}» در فهرستِ استثنایِ صدای لمس هست ولی در رابط ساخته نمی‌شود`);
    }
    notes.push(`صدای لمس: ${classes.length} ناحیهٔ مستثنا، همه موجود`);
  }
}
