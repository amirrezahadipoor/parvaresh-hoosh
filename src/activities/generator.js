...
window.Generator = (function() {
    ...
    const READING_STORIES = [
        {
            tags: 'صبح مدرسه روز بارانی',
            text: 'صبح باران می‌بارید. سارا چترش را برداشت و با پدر به مدرسه رفت. او کفش‌های خیسش را کنار در گذاشت.',
            questions: [
                { q: 'هوا چگونه بود؟', a: 'بارانی', wrong: ['برفی', 'آفتابی'] },
                { q: 'سارا چه چیزی برداشت؟', a: 'چتر', wrong: ['توپ', 'کتاب داستان'] },
                { q: 'سارا با چه کسی رفت؟', a: 'پدر', wrong: ['دوستش', 'مادربزرگ'] }
            ]
        },
        {
            tags: 'باغچه گیاه',
            text: 'امیر یک دانه در باغچه کاشت. هر روز کمی آب به آن داد. چند روز بعد، جوانهٔ سبزی از خاک بیرون آمد.',
            questions: [
                { q: 'امیر چه چیزی کاشت؟', a: 'دانه', wrong: ['سنگ', 'مداد'] },
                { q: 'او هر روز چه کار کرد؟', a: 'به دانه آب داد', wrong: ['دانه را دور انداخت', 'روی خاک نمک ریخت'] },
                { q: 'در پایان چه چیزی رویید؟', a: 'جوانهٔ سبز', wrong: ['اسباب‌بازی', 'قارچ بزرگ'] }
            ]
        },
        {
            tags: 'دوست تازه مهربان شخصیت',
            text: 'نیما در کلاس تازه تنها بود. آراد کنارش نشست، نامش را پرسید و او را به بازی دعوت کرد. نیما لبخند زد.',
            questions: [
                { q: 'چه کسی تنها بود؟', a: 'نیما', wrong: ['آراد', 'معلم'] },
                { q: 'آراد چه کار مهربانی انجام داد؟', a: 'نیما را به بازی دعوت کرد', wrong: ['از او دور شد', 'اسباب‌بازی‌اش را پنهان کرد'] },
                { q: 'چرا نیما لبخند زد؟', a: 'چون یک دوست پیدا کرد', wrong: ['چون کلاس تعطیل شد', 'چون خوابش برد'] }
            ]
        },
        {
            tags: 'پارک سفر',
            text: 'مینا با مادر به پارک رفت. اول به پرنده‌ها نگاه کرد، بعد تاب بازی کرد و در پایان زباله‌اش را در سطل انداخت.',
            questions: [
                { q: 'مینا کجا رفت؟', a: 'پارک', wrong: ['فروشگاه', 'بیمارستان'] },
                { q: 'بعد از دیدن پرنده‌ها چه کرد؟', a: 'تاب بازی کرد', wrong: ['به خانه خوابید', 'کتاب خرید'] },
                { q: 'در پایان زباله را کجا انداخت؟', a: 'در سطل', wrong: ['روی چمن', 'کنار تاب'] }
            ]
        }
    ];

    function comprehensionRound(topic) {
        const query = String(topic || '');
        const matching = READING_STORIES.filter(story => story.tags.split(' ').some(tag => query.includes(tag)));
        const story = pick(matching.length ? matching : READING_STORIES);
        const question = pick(story.questions);
        const options = shuffle([question.a, ...question.wrong]);
        return mc(
            question.q,
            `<div class="reading-story-card" dir="rtl">${story.text}</div>`,
            options.map(label => ({ label })),
            options.indexOf(question.a),
            'داستان را بخوان و پاسخ درست را انتخاب کن'
        );
    }

    function grammarRound(kind) {
        const banks = {
            plural: [
                { q: 'جمع «کتاب» کدام است؟', a: 'کتاب‌ها', wrong: ['کتابی', 'کتابم'] },
                { q: 'جمع «درخت» کدام است؟', a: 'درخت‌ها', wrong: ['درختی', 'درختم'] },
                { q: 'جمع «پرنده» کدام است؟', a: 'پرنده‌ها', wrong: ['پرندگی', 'پرنده‌ام'] }
            ],
            pair: [
                { q: 'جفت مناسب «پدر» کدام است؟', a: 'مادر', wrong: ['برادر', 'پدربزرگ'] },
                { q: 'جفت مناسب «برادر» کدام است؟', a: 'خواهر', wrong: ['عمو', 'پدر'] },
                { q: 'جفت مناسب «خروس» کدام است؟', a: 'مرغ', wrong: ['اردک', 'جوجه'] }
            ],
            nounVerb: [
                { q: 'کدام واژه یک کار را نشان می‌دهد؟', a: 'می‌دود', wrong: ['کتاب', 'درخت'] },
                { q: 'کدام واژه نام یک چیز است؟', a: 'مداد', wrong: ['می‌نویسد', 'می‌خندد'] },
                { q: 'در جملهٔ «پرنده پرواز می‌کند» فعل کدام است؟', a: 'پرواز می‌کند', wrong: ['پرنده', 'آسمان'] }
            ],
            family: [
                { q: 'کدام واژه با «دانش» هم‌خانواده است؟', a: 'دانشمند', wrong: ['باران', 'پنجره'] },
                { q: 'کدام واژه با «آموزش» هم‌خانواده است؟', a: 'آموزگار', wrong: ['خورشید', 'دوچرخه'] },
                { q: 'کدام واژه با «نویس» هم‌خانواده است؟', a: 'نویسنده', wrong: ['پرنده', 'رودخانه'] }
            ],
            verb: [
                { q: 'ماهی در آب ... .', a: 'شنا می‌کند', wrong: ['می‌نویسد', 'می‌کارد'] },
                { q: 'پرنده در آسمان ... .', a: 'پرواز می‌کند', wrong: ['آب می‌دهد', 'می‌پزد'] },
                { q: 'مادر کتاب را ... .', a: 'می‌خواند', wrong: ['می‌بارد', 'می‌روید'] }
            ],
            punctuation: [
                { q: 'پایان جملهٔ پرسشی کدام نشانه است؟', a: '؟', wrong: ['.', '،'] },
                { q: 'پایان جملهٔ خبری کدام نشانه است؟', a: '.', wrong: ['؟', '،'] },
                { q: 'در «سلام، دوست من» بعد از سلام چه می‌آید؟', a: '،', wrong: ['؟', '.'] }
            ]
        };
        const item = pick(banks[kind] || banks.nounVerb);
        const options = shuffle([item.a, ...item.wrong]);
        return mc(item.q, SvgArt.questionTile('#6C5CE7', 100), options.map(label => ({ label })), options.indexOf(item.a), 'به جمله و واژه‌ها دقت کن و پاسخ درست را پیدا کن');
    }
    ...
    function paintingRound() {
        return {
            ...
            speech: 'هر چیزی که دوست داری با رنگ‌های شاد نقاشی کن!'
        };
    }
    ...
    function simpleSubstitute(domainHint) {
        ...
        const pools = {
            reading: [() => letterSoundRound(pick(ALPHABET)), () => letterExampleRound(pick(ALPHABET)), () => tracingRound(pick(ALPHABET).letter, 'letter')],
            math: [() => countRound(5), () => numberNameRound(5), () => compareRound(5), () => tracingRound(toFaDigit(rint(1, 5)), 'number'), shapeMatchRound],
            logic: [() => memoryRound(2), shapeMatchRound, () => oddOneOutRound('shapes')],
            science: [animalSoundRound],
            'socio-emotional': [() => emotionRound(true)],
            art: [paintingRound, colorRound, patternRound],
            gifted: [() => memoryRound(2), shapeMatchRound, () => oddOneOutRound('shapes')]
        };
        return pick(pools[domain] || [shapeMatchRound])();
    }
    ...
    function advancedSubstitute(domainHint) {
        ...
        return pick(pools[domain] || [ravenRound])();
    }
    ...
    function retypeRoundForTrack(round, trackKey, domainHint, childAge) {
        ...
        if (READ_HEAVY.indexOf(round.type) !== -1 && (promptTooLong ||
            ...
            } catch (e) { /* keep the original only if construction fails */ }
        }
    ...
    function titlePlan(metadata, level) {
        ...
        if (inDomain('reading') && (has('مفرد و جمع')))
            return [() => grammarRound('plural'), () => grammarRound('plural'), () => grammarRound('plural'), () => grammarRound('plural')];
        if (inDomain('reading') && (has('نام زن و مرد', 'مذکر و مؤنث')))
            return [() => grammarRound('pair'), () => grammarRound('pair'), () => grammarRound('pair'), () => grammarRound('pair')];
        if (inDomain('reading') && (has('اسم و فعل')))
            return [() => grammarRound('nounVerb'), () => grammarRound('nounVerb'), () => grammarRound('nounVerb'), () => grammarRound('nounVerb')];
        if (inDomain('reading') && (has('هم‌خانواده', 'هم خانواده')))
            return [() => grammarRound('family'), () => grammarRound('family'), () => grammarRound('family'), () => grammarRound('family')];
        if (inDomain('reading') && (has('فعل مناسب')))
            return [() => grammarRound('verb'), () => grammarRound('verb'), () => grammarRound('verb'), () => grammarRound('verb')];
        if (inDomain('reading') && (has('نشانه‌گذاری', 'پرسشی و خبری')))
            return [() => grammarRound('punctuation'), () => grammarRound('punctuation'), () => grammarRound('punctuation'), () => grammarRound('punctuation')];
        if (inDomain('reading') && (has('داستان', 'داستانک', 'پاراگراف', 'چه کسی چه کاری', 'معنی درست', 'رفتار شخصیت', 'نتیجه داستان')))
            return [() => comprehensionRound(title), () => comprehensionRound(title), () => comprehensionRound(title), () => comprehensionRound(title)];
        ...
        if (inDomain('reading') && (has('صدای آخر', 'صدای مشترک')))
        ...
        if (inDomain('reading') && (has('ترتیب واژه', 'مرتب کردن کلمات', 'جمله سازی', 'جمله‌سازی', 'جمله‌های کوتاه', 'جمله‌های کمی بلندتر')))
        ...
        if (inDomain('math') && (has('اندازه‌گیری', 'طول', 'کوتاه و بلند')))
            return [orderSizeRound, () => compareRound(level <= 4 ? 10 : 20), orderSizeRound, balanceRound];
    ...
    function buildRounds(lessonId, metadata) {
        // Direct subject and adventure entry share one authoritative resolver.
        // Title-specific content always wins; generic progressive/package plans
        // are fallbacks and can never inject a foreign activity into the subject.
        const lvl = lessonLevel(lessonId, metadata);
    ...
    function generate(lessonId, metadata) {
        ...
        // audioClip is always the exact actionable instruction. Topic narration
        // lives only in lessonIntro, so «بشنو» can never replay a generic subject
        // welcome instead of the current question.
        list.forEach(round => {
            const isLetterSound = String(round.audioClip || '').startsWith('letter-')
                && /کدام حرف/.test(String(round.prompt || ''));
            if (isLetterSound) return;
