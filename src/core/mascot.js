// Multi-Character Mascot Engine for "پرورش هوش کودک"
// Cast: Dana (Fox), Barfi (Rabbit), Shiri (Lion Cub), Toto (Parrot), Fandogh (Squirrel)
window.Mascot = (function() {

    const CHARACTERS = [
        { id: 'dana', name: 'دانا (روباه باهوش)', title: 'راهنمای دانا', color: '#FF763B' },
    ...
    function svgFox(size, mood, uid) {
        ...
        if (mood === 'celebrating' || mood === 'proud') {
            ...
            extras = `<g transform="translate(68, 6)"><circle cx="10" cy="11" r="5" fill="#A29BFE"/><circle cx="20" cy="3" r="7.5" fill="#6C5CE7"/><text x="20" y="7" font-size="10" font-weight="900" fill="#FFF" text-anchor="middle">؟</text></g>`;
        }

        return `
        <svg id="${uid}" class="mascot-vector mascot-${mood}" width="${size}" height="${size}" viewBox="0 0 100 100" style="overflow:visible;">
            <defs>
    ...
    function svgParrot(size, mood, uid) {
        ...
        <svg id="${uid}" class="mascot-vector mascot-${mood}" width="${size}" height="${size}" viewBox="0 0 100 100" style="overflow:visible;">
            ...
            <path d="M22 46 Q10 60 22 74" fill="#FF4757" stroke="#2C3A47" stroke-width="2"/>
            <path d="M78 46 Q90 60 78 74" fill="#FF4757" stroke="#2C3A47" stroke-width="2"/>
    ...
    function svgSquirrel(size, mood, uid) {
        return `
        <svg id="${uid}" class="mascot-vector mascot-${mood}" width="${size}" height="${size}" viewBox="0 0 100 100" style="overflow:visible;">
            <path d="M70 66 Q98 68 92 40 Q84 20 68 34 Q58 46 70 66 Z" fill="#D35400" stroke="#2C3A47" stroke-width="2.5"/>
            <path d="M26 32 L22 14 L38 24 Z" fill="#E67E22" stroke="#2C3A47" stroke-width="2"/>
            <path d="M74 32 L78 14 L62 24 Z" fill="#E67E22" stroke="#2C3A47" stroke-width="2"/>
