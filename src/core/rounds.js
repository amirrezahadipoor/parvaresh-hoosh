...
const CALM_MOVES = Object.freeze([
  ...
  { feeling: 'متعجب', move: 'خوب نگاه کن', why: 'وقتی چیزی تازه می‌بینی، خوب نگاه کردن جواب را پیدا می‌کند.' },
  // ⚠ «شاد» و «آرام» چهره دارند ولی حرکتِ آرام‌سازی نداشتند، پس در
  // این گِرد همیشه بدل بودند و هرگز پاسخ نمی‌شدند — همان اشتباهی که
  // یک بار در «عصبانی» و «متعجب» رخ داد. حرکتِ درست برای حالِ خوب
  // «آرام کردن» نیست، «نگه داشتن و سهیم شدن» است.
  { feeling: 'شاد', move: 'با دوستت قسمت کن', why: 'شادی وقتی قسمت شود، بزرگ‌تر می‌شود.' },
  { feeling: 'شاد', move: 'بخند و بازی کن', why: 'وقتی حالت خوب است، بازی حالت را خوب‌تر می‌کند.' },
  { feeling: 'آرام', move: 'کتاب بخوان', why: 'وقتی آرامی، بهترین وقتِ یاد گرفتن است.' },
  { feeling: 'آرام', move: 'به کسی کمک کن', why: 'وقتی خودت آرامی، می‌توانی حواست به دیگران باشد.' },
]);
...
function buildRoundInner(round, track) {
  ...
  switch (round.kind) {
    case 'letter-sound': {
      ...
      const pic = pictureFor(round.answer);
      // اموجی بلندگو چیزی یاد نمی‌دهد، ولی نشان دادن *خودِ حرف* هم
      // پرسش را بی‌معنا می‌کند: کودک فقط کپی می‌کند. پس تصویرِ
      // کلمه را نشان می‌دهیم — کودک باید از تصویر به حرف برسد.
      //
      // ⚠ اما متنِ پرسش باید با آنچه روی صفحه است بخواند. نسخهٔ
      // قبلی می‌پرسید «کدام یکی حرف الف است؟» و تصویرِ *انار* را
      // نشان می‌داد. برای کودکِ باصدا منطقی بود (صدا را می‌شنید و
      // تصویر فقط یاری‌رسان بود)، ولی برنامه باید **بدون هیچ صدایی**
      // کامل کار کند — و بی‌صدا، آن صفحه یک انارِ بی‌ربط کنارِ یک
      // پرسشِ حرفی بود. کودک نمی‌فهمید تصویر آنجا چه می‌کند.
      //
      // حالا وقتی تصویری هست، پرسش دربارهٔ همان تصویر است.
      // ⚠ نخست نوشتم «با کدام حرف شروع می‌شود؟» و بلافاصله غلط از آب
      // درآمد: LETTER_PICTURE جدولِ *شروع* نیست، جدولِ «حرف در این
      // کلمه هست» است — «ب» در «ابر»، «ن» در «خانه»، «ع» در «شمع».
      // این عمدی است، چون برای همهٔ ۳۲ حرف کلمهٔ آغازینِ تصویردار
      // وجود ندارد. پس پرسش هم باید همان را بپرسد.
      const prompt = pic ? `کدام حرف در «${pic}» هست؟` : round.prompt;
      return {
        type: 'choice',
        prompt,
        speak: round.speak,
        display: pic ? { kind: 'pic-only', icon: pic } : null,
        options: buildOptions(round.answer, pool, n).map((v) => ({ label: v, value: v })),
        answer: round.answer,
        because: pic ? `حرف «${round.answer}» در «${pic}» هست.` : undefined,
      };
    }
    ...
    case 'measure-units': {
      ...
      // ⚠ سه واحد × پنج طول = پانزده نما، ولی گاردِ کم‌تنوعی (که
      // برچسبِ گزینه‌ها را هم می‌شمارد) فقط ۴ نما می‌دید: با چهار
      // گزینه، مجموعهٔ اعدادِ روی صفحه تقریباً همیشه یکی بود. راهِ
      // درست بیشتر کردنِ *واحد* است، چون واحد چیزی است که کودک
      // می‌بیند و مفهومِ درس هم دقیقاً همین است: «واحد را عوض کن،
      // عدد عوض می‌شود».
      const unit = pick(['گیره', 'مداد', 'پاک‌کن', 'قاشق', 'کلید', 'سکه']);
    ...
    case 'safety-order': {
      ...
      const CHAINS = [
        ...
        {
          // آتش یا دود: دور شو، بیرون برو، به بزرگ‌تر بگو
          steps: ['دورشدن', 'خانه', 'گفتن‌به‌بزرگ‌تر'],
          labels: { دورشدن: 'دور شو', خانه: 'بیرون', 'گفتن‌به‌بزرگ‌تر': 'بگو' },
        },
        {
          // سگِ ناآشنا: بایست، آرام باش، به بزرگ‌تر بگو
          steps: ['دورشدن', 'نفس‌عمیق', 'گفتن‌به‌بزرگ‌تر'],
          labels: { دورشدن: 'بایست', 'نفس‌عمیق': 'آرام', 'گفتن‌به‌بزرگ‌تر': 'بگو' },
        },
        {
          // گم شدن: همان‌جا بمان، صدا بزن، از بزرگ‌تر کمک بخواه
          steps: ['گم‌شدن', 'گفتن‌به‌بزرگ‌تر', 'کمک‌کردن'],
          labels: { 'گم‌شدن': 'بمان', 'گفتن‌به‌بزرگ‌تر': 'صدا بزن', 'کمک‌کردن': 'کمک' },
        },
        {
    ...
    case 'event-order': {
      ...
      const CHAINS = [
        ...
        { name: 'صبح', steps: ['خواب', 'مسواک', 'کتاب‌خواندن'], labels: ['بیدار شو', 'مسواک', 'کتاب'] },
        { name: 'بازی', steps: ['دویدن', 'آب‌خوردن', 'خواب'], labels: ['بدو', 'آب بخور', 'بخواب'] },
        { name: 'گیاه', steps: ['دانه', 'باران', 'گل'], labels: ['دانه', 'آب', 'گل'] },
        { name: 'تمیزی', steps: ['غذا', 'دستمال', 'زبالهٔ‌درست'], labels: ['بخور', 'پاک کن', 'در سطل'] },
        { name: 'دوستی', steps: ['تنهایی', 'دوستِ‌تازه', 'توپ'], labels: ['تنهاست', 'سلام کن', 'بازی'] },
      ];
